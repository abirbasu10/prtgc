	...
	if(isset($_GET['apicall'])){
		...
		switch($_GET['apicall']){
			...
			case 'signup':
				...
				if($this->isTheseParametersAvailable(array('username','email','password','gender'))){
			...
			case 'login':
				...
				//echo $this->isTheseParametersAvailable(array('username', 'password'));
				...
				//	$stmt->store_result();
					...
					$result = DB::select("SELECT id, username, email, gender FROM users WHERE username = ? AND password = ?",[$username,$password]);
				...
				//	if($stmt->num_rows > 0){
					...
					//	$stmt->fetch();
						...
						//	echo "User Found".$result[0]->email;
					}else{
...
public	function isTheseParametersAvailable(array $params){
